...
later(function()
  ...
  ai.setup({
    ...
    custom_textobjects = {
      ...
      -- Make `ad` / `id` act on around/inside whole *d*igits
      d = MiniExtra.gen_ai_spec.number(),
      -- Make `aD` / `iD` act on around/inside whole *D*iagnostic
      D = MiniExtra.gen_ai_spec.diagnostic(),
      -- Make `aj` / `ij` act on around/inside whole line
      j = { "%f[^%c][^%c]*", "^%s*().-()%s*$" },
...
later(function()
  ...
  MiniKeymap.map_multistep("i", "<Tab>", { "pmenu_next", "increase_indent", "jump_after_close" })
